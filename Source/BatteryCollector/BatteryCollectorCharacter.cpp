...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
	...
	//Создаем зону сбора объектов при создании персонажа 
...
{
	// Получаем все объект попавшие в зону сбора и записываем их в массив
	TArray<AActor*> CollectedActors;
	CollectionSphere->GetOverlappingActors(CollectedActors);

	// Amount of the collected battery's power
	...
	for (int32 iCollected = 0; iCollected < CollectedActors.Num(); ++iCollected)
	{
		...
		// If the cast is successful and the pick is valid and active
		if (TestPickup && !TestPickup->IsPendingKill() && TestPickup->IsActive())
		{
			// Call th pickup's WasCollected function
			TestPickup->WasCollected();

			// Check to see if the pickup is also a battery
			ACauldronPickup* const TestCauldron = Cast<ACauldronPickup>(TestPickup);

			if (TestCauldron)
			{
				CollectedPower += TestCauldron->GetPower();
			}
...
float ABatteryCollectorCharacter::GetCurrentPower() { return CurrentPower; }

// Called whenever power is increased or decreased
void ABatteryCollectorCharacter::UpdatePower(float CharacterPowerChange) 
{ 
	// ChangePower
	if (CurrentPower <= InitialPower * 1.25f) 
	{
		CurrentPower += CharacterPowerChange * 10;
	}
